...
all_sig_comp_BR<-function(sub_set){
  ...
  for (j in 1:n) {
    ...
    results_wilcoxon<-wilcox.test(data_set$currentscore ~ data_set$response)
...
pdf(paste0("./Figures_print/FigS5Ib_Bxplt_Risk.pdf"),width =6, height = 7.5)
all_sig_comp_BR("_Inf")
dev.off()

saveWorkbook(wb, "./Figure 5S/FigS5Ib_Bxplt_prgm_stats.xlsx", overwrite = TRUE)
