...
ttau_123<-table(df_CD_Infl_Pre_Ileum$sample_id,df_CD_Infl_Pre_Ileum$annot)[-11,c(5:7)]
gd_ttau<-ttau[which(rowSums(ttau)>9),]
gd_ttau_123<-ttau_123[which(rowSums(ttau_123)>9),]
gd_ratiom1m3_tau<-data.frame(prop_m1=prop.table(gd_ttau,margin=1)[,1],origin="taurus") 
gd_ratiom1m2m3_tau<-data.frame(prop_m1=prop.table(gd_ttau_123,margin=1)[,1],origin="taurus") 

gd_ratiom1m3<-rbind(gd_ratiom1m3_ibd,gd_ratiom1m3_tau)
gd_ratiom1m2m3<-rbind(gd_ratiom1m3_ibd,gd_ratiom1m2m3_tau)



palette<-c("#999999","#000000")
palette2<-c("black","white")
gd_ratiom1m3$category<-factor(gd_ratiom1m3$origin, levels=c("taurus","nan_sin"))
gd_ratiom1m2m3$category<-factor(gd_ratiom1m2m3$origin, levels=c("taurus","nan_sin"))

wilc_rstatix<-gd_ratiom1m3 %>%rstatix::wilcox_test(prop_m1 ~ category)
wilc_rstatix2<-gd_ratiom1m2m3 %>%rstatix::wilcox_test(prop_m1 ~ category)
df_pvals <- wilc_rstatix %>% add_xy_position() %>%
  add_significance("p") %>%mutate(category="taurus") 
df_pvals2 <- wilc_rstatix2 %>% add_xy_position() %>%
  add_significance("p") %>%mutate(category="taurus") 
df_pvals$y.position<-df_pvals$y.position+0.05 
df_pvals2$y.position<-df_pvals$y.position+0.05 
df_pvals$p_round<-round(df_pvals$p,4)
df_pvals2$p_round<-round(df_pvals2$p,4)

prismplot2<-function(long_df2,y_title,y_val,p_v_lab,psiz=25){
  ggplot(long_df2, aes(x =category, y =!!sym(y_val), fill=category))+
    ...
    scale_color_manual(values = palette)+ scale_y_continuous(breaks = c(0.25,0.5,0.75,1)) +
    ...
    coord_cartesian(ylim = c(0, 1.1),clip = 'off')#+  #cut the plot at 1
}

pdf("./Figures_print/Fig2A_Bxplt_M1prop.pdf",width =4.5,height = 7.5)
prismplot2(gd_ratiom1m3,y_title="IFIM proportion in IFIM+IRM",y_val="prop_m1",p_v_lab="p.signif")
prismplot2(gd_ratiom1m3,y_title="IFIM proportion in IFIM+IRM",y_val="prop_m1",p_v_lab="p_round",psiz=10)
prismplot2(gd_ratiom1m2m3,y_title="IFIM proportion in Infl mono",y_val="prop_m1",p_v_lab="p.signif")
prismplot2(gd_ratiom1m2m3,y_title="IFIM proportion in Infl mono",y_val="prop_m1",p_v_lab="p_round",psiz=10)
dev.off()


# Statistics details:
wilcox.test(gd_ratiom1m3$prop_m1 ~ gd_ratiom1m3$origin)
wilcox.test(gd_ratiom1m2m3$prop_m1 ~ gd_ratiom1m2m3$origin)
