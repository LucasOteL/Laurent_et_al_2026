...
for(mods in names(Modsig)[-length(names(Modsig))]){
  eval(parse(text=paste0("ilist<-Modsig$",mods)))
...
#In parallel to the figures, create a spreadsheet with stat results:
wb <- createWorkbook()
addWorksheet(wb, "KW_Wilcox_Signatures")
xl_row <- 1

pdf(file="./Figures_print/Fig1E_S2C_Bxplt_prgm.pdf",width = 7, height = 6)
for(mods in names(Modsig)[-length(names(Modsig))]){
  eval(parse(text=paste0("ilist<-Modsig$",mods)))
  ...
  # Stats : check K-W result, then do wilcoxon signed rank of group median vs global median + correction
  KW_rstatix<-df_zmod3 %>%rstatix::kruskal_test(currentscore ~ Annotation)
  
  results_wilcoxon<- df_zmod3 %>% dplyr::group_by(Annotation) %>% 
    # dplyr::summarise(p_value = wilcox.test(currentscore, mu = median(df_zmod3$currentscore))$p.value, .groups = "drop") %>% 
    dplyr::summarise(p_value = wilcox.test(currentscore, mu = mean(df_zmod3$currentscore))$p.value, .groups = "drop") %>%
    dplyr::mutate(p_adj = p.adjust(p_value, method = "bonferroni"))
  
  old.cluster.ids<-c("Mono-like 1","Mono-like 2","Mono-like 3","Mono-like 4","Mono-like 5",
  ...
  results_wilcoxon$Annotation<- plyr::mapvalues(x = results_wilcoxon$Annotation, from = old.cluster.ids, to = new.cluster.ids)
  results_wilcoxon<-results_wilcoxon[match(new.cluster.ids,results_wilcoxon$Annotation),] #Reorder like the plot
  
  # Save results for spreadsheet:
  writeData(wb, "KW_Wilcox_Signatures", paste0(mods), startRow = xl_row, colNames = FALSE)
  xl_row <- xl_row + 1
  writeData(wb, "KW_Wilcox_Signatures", paste0("Kruskal-Wallis:"), startRow = xl_row, colNames = FALSE)
  xl_row <- xl_row + 1
  writeData(wb, "KW_Wilcox_Signatures", KW_rstatix[,2:5], startRow = xl_row)
  xl_row <- xl_row + nrow(KW_rstatix) + 1
  writeData(wb, "KW_Wilcox_Signatures", paste0("Signature global median:"), startRow = xl_row, colNames = FALSE)
  xl_row <- xl_row + 1
  writeData(wb, "KW_Wilcox_Signatures",  median(df_zmod3$currentscore), startRow = xl_row)
  xl_row <- xl_row + 1
  writeData(wb, "KW_Wilcox_Signatures", paste0("Wilcoxon signed rank test:"), startRow = xl_row, colNames = FALSE)
  xl_row <- xl_row + 1
  writeData(wb, "KW_Wilcox_Signatures", results_wilcoxon, startRow = xl_row)
  xl_row <- xl_row + nrow(results_wilcoxon) + 2  # 2 lines as separator row
  ...
  print( plot_w4+
           ...
           # stat_summary(fun.y=mean, geom="point", shape=20, size=4, color="red", fill="red") +  # To see the mean
           scale_color_manual(values = palette_en_cours2) +coord_cartesian(ylim = c(minval, (maxval+ 0.2*bx_stat_siz)))+
           theme(axis.text.x = element_blank(),
                 # axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5,size = rel(1.8),margin = margin(t = 6)),  # If we add x labels
           ...
           geom_hline(yintercept = median(df_zmod3$currentscore), linetype = "dashed",color=clr_ln_score, size=rel(0.7)*bx_siz) +
...
saveWorkbook(wb, "./Figure 1/Fig1E_S2C_Bxplt_prgm_stats.xlsx", overwrite = TRUE)
