...
maxval=0
for(mods in pthw){
  ilist<-intersect(model_500[which(model_500$pathway==mods&model_500$weight>0),]$gene,rownames(preloco_nc))
...
pdf(file="./Figures_print/Fig4D_S10A_Bxplt_prgm_progeny.pdf",width = 8, height = 5.5)
for(mods in pthw){
  ilist<-intersect(model_500[which(model_500$pathway==mods&model_500$weight>0),]$gene,rownames(preloco_nc))
  ...
  writeData(wb, "KW_Wilcox_Signatures", paste0("Wilcoxon test:"), startRow = xl_row, colNames = FALSE)
  xl_row <- xl_row + 1
...
saveWorkbook(wb, "./Figure 4/Fig4D_S10A_Bxplt_progeny_stats.xlsx", overwrite = TRUE)
